...
const MobileProjectCard = ({ item }) => {
  ...
  return (
    <motion.div 
      className="relative w-full aspect-[4/3] rounded-[24px] overflow-hidden bg-[#222] cursor-none"
      whileHover="hover"
      initial="initial"
      onMouseMove={handleMouseMove}
      onMouseEnter={handleMouseEnter}
    >
      ...
      <motion.div
        className="absolute inset-0 z-30 flex flex-col p-8 justify-center pointer-events-none"
        style={{ backgroundColor: item.campaignColor }}
        variants={{ initial: { opacity: 0 }, hover: { opacity: 1 } }}
        transition={{ duration: 0.4 }}
      >
        <motion.h3
          className="text-[32px] leading-[1] tracking-[-1.5px] text-[#111] font-semibold"
      ...
      <motion.div
        className="absolute top-0 left-0 z-40 pointer-events-none w-20 h-20 bg-[#abf0d1] rounded-full flex items-center justify-center text-black shadow-2xl"
        style={{ x: springX, y: springY, translateX: "-50%", translateY: "-50%", originX: 0.5, originY: 0.5 }}
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-black/10 z-10 pointer-events-none"></div>

      <div className="absolute top-4 right-4 bg-white/20 backdrop-blur-xl px-4 py-2 rounded-full flex items-center gap-2 text-white font-medium z-20 pointer-events-none">
        <Search className="w-3.5 h-3.5" />
        <span className="text-[13px]">{item.tag}</span>
        <TrendingUp className="w-3.5 h-3.5" />
      </div>

      <div className="absolute bottom-5 left-5 flex flex-col z-20 pointer-events-none">
        <span className="text-[14px] font-bold text-white mb-1.5">{item.year}</span>
        <h2 className="text-[40px] leading-[0.9] tracking-[-1px] font-bold text-white">
          {item.title}
        </h2>
      </div>
    </motion.div>
    ...
    <div className="w-full px-3 md:px-4 flex justify-center">
      ...
      >
        ...
        <div className="md:hidden flex flex-col relative z-10 pb-10">
          <p className="text-[20px] font-medium mb-6 tracking-tight text-white/80">Featured Work</p>

          <div className="flex flex-col gap-6">
            {works.map((item, index) => (
              <MobileProjectCard key={index} item={item} />
            ))}
          </div>
        </div>
